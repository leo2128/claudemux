...
4. **Deliverable shape.** What kind of artifact you want back: a verdict, a PR/MR, a written report, a set of file edits, a one-line answer. Without this, teammates sometimes return the wrong artifact shape. When the deliverable is a PR, also tell the teammate to include a changeset fragment — every PR to this repo that changes plugin source must ship a `plugins/claudemux/.changeset/<slug>.md` file declaring the bump level; see CLAUDE.md § "feishu-channel versioning" for the format. Write the file directly; do not use the interactive `changeset` CLI.
